...
int ext2_create(EXT2_NODE *parent, const char *entryName, EXT2_NODE *retEntry){
    ...
    if(format_name(parent->fs,name)==EXT2_ERROR) return EXT2_ERROR-1;

    ZeroMemory(retEntry,sizeof(EXT2_NODE));
    memcpy(retEntry->entry.name,entryName,MAX_ENTRY_NAME_LENGTH);
    if(ext2_lookup(parent,name,retEntry)==EXT2_SUCCESS) return EXT2_ERROR-2;
...
int read_dir_from_block(EXT2_FILESYSTEM *fs,BYTE *block,EXT2_NODE_ADD adder,void *list){
    ...
    //printf("dir3%s",dir_entry[3].name);
    //printf("hh\n");
    while(dir_entry[i].name[0]!=NULL){
        //printf("\n%d:%s\n",i,dir_entry[i].name);
